...
lowStockItems 低庫存商品陣列中的值。即可印出說明文字與lowStockItems 低庫存商品陣列中的值。
```js
    const products = [             
...
Ans:
- 定義兩個陣列，定義兩個陣列，包含商品庫存 products 與更新庫存 updates ，並利⽤ for 進⾏巢狀迴圈來逐⼀判斷每個商品項
...
Ans:
- 使⽤ switch 敘述完成路由判斷。使⽤ switch 敘述完成路由判斷。若url值為'/'，則輸出顯⽰'index.html輸出部分'字樣；若url值為'/calculator'，
...
Ans:
- ⾸先必須先引入需要⽤到的模組後，⾸先必須先引入需要⽤到的模組後，並建立伺服器，開始定義對應的 MIME 類型，且利⽤ switch 敘述來設定路
...
Ans:
- 在原本處理靜態檔案的程式區塊，在原本處理靜態檔案的程式區塊新增以下程式；在原本的 fs.readFile 處理非同步讀取檔案的地⽅，修改
